...
package body MVM.ASM is
   ...
   procedure Load_Code_From_File (C : in out Code; Src_File : in out File_Type)
   is
      F          : Positive;
      L          : Natural;
   ...
   begin
      ...
      while not End_Of_File (Src_File) loop
         declare
            Current_Line : constant String := Get_Line (Src_File);
         begin
            Find_Token
              (Source => Current_Line,
               Set    => Whitespace,
            ...
            if Current_Line (F .. L) = "load" then
               ...
               Find_Token
                 (Source => Current_Line,
                  ...
                  First  => F,
                  Last   => L);
               Address := Instruction_Address'Value (Current_Line (F .. L));
